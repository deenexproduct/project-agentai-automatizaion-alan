...
export default function PremiumHeader({
    ...
    return (
        <div className={`flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4 bg-white/80 backdrop-blur-2xl shrink-0 relative z-10 border-b border-white/50 shadow-[0_4px_20px_rgba(30,27,75,0.03)] ${containerClassName || 'px-6 py-4'}`}>
            {/* Search Bar - Expansivo y protagonista, versión más fina (finito) */}
            <div className="relative group/search flex-1 w-full flex items-center">
                {/* Subtle animated background line on focus */}
            ...
            <div className="flex items-center gap-3 shrink-0 ml-auto">
                {onFilter && (
                    <button
                        onClick={onFilter}
                        className={`group relative flex items-center justify-center h-[42px] w-[42px] rounded-[14px] transition-all duration-300 border overflow-hidden ${showFilters ? 'bg-violet-50 border-violet-200 text-violet-600 shadow-[inset_0_2px_4px_rgba(139,92,246,0.1)]' : 'bg-white border-slate-200/60 text-slate-500 hover:text-violet-600 shadow-[0_2px_10px_rgba(0,0,0,0.02)] hover:shadow-[0_8px_20px_rgba(139,92,246,0.08)] hover:-translate-y-0.5 hover:border-violet-200'}`}
                        title="Filtros"
                    >
